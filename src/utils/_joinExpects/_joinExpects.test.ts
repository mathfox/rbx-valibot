...
import { _joinExpects } from "./_joinExpects";

describe("_joinExpects", () => {
	test("should remove duplicates", () => {
		expect(_joinExpects(['"foo"', '"foo"'], "|")).toBe('"foo"');
		expect(_joinExpects(['"foo"', "string", '"foo"'], "&")).toBe('("foo" & string)');
	});

	test("should join multiple items", () => {
		expect(_joinExpects(['"foo"', "string"], "&")).toBe('("foo" & string)');
		expect(_joinExpects(['"foo"', "number", "boolean", "Object"], "|")).toBe('("foo" | number | boolean | Object)');
	});
