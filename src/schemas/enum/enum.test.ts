...
describe("enum_", () => {
	...
	describe("should return dataset without issues", () => {
		test("for valid options", () => {
			expectNoSchemaIssue(enum_(options), [options.option1, options.option2, options.option3]);
		});
	...
	describe("should return dataset with issues", () => {
		...
		test("for empty options", () => {
			enum Empty {}
			expectSchemaIssue(enum_(Empty, "message"), { ...baseIssue, expected: "never" }, ["foo", "bar", "baz"]);
		});
