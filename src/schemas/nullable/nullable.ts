...
> extends BaseSchema<InferInput<TWrapped> | null, InferNullableOutput<TWrapped, TDefault>, InferIssue<TWrapped>> {
	/**
	 * The schema type.
	 */
 ...
 */
export function nullable<const TWrapped extends BaseSchema<unknown, unknown, BaseIssue<unknown>>>(
	wrapped: TWrapped,
): NullableSchema<TWrapped, never>;
...
export function nullable(
	...
	// @ts-expect-error
	const schema: NullableSchema<BaseSchema<unknown, unknown, BaseIssue<unknown>>, unknown> = {
		kind: "schema",
		type: "nullable",
		reference: nullable,
		...
		_run(dataset, config) {
			...
			if (dataset.value === null) {
				...
				if ("default" in this) {
					dataset.value = getDefault(this, dataset as Dataset<null, never>, config);
				}
