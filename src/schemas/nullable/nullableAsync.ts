...
> extends BaseSchemaAsync<InferInput<TWrapped> | null, InferNullableOutput<TWrapped, TDefault>, InferIssue<TWrapped>> {
	/**
	 * The schema type.
	 */
...
>(wrapped: TWrapped, default_: TDefault): NullableSchemaAsync<TWrapped, TDefault>;
	...
	> = {
		...
		async _run(dataset, config) {
			...
			if (dataset.value === null) {
				...
				if ("default" in this) {
					dataset.value = await getDefault(this, dataset as Dataset<null, never>, config);
				}
