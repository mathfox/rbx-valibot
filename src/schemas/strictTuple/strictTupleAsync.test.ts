...
import { type StrictTupleSchemaAsync, strictTupleAsync } from "./strictTupleAsync.ts";
...
describe("strictTupleAsync", () => {
	...
	describe("should return dataset without issues", () => {
		...
		test("for simple tuple", async () => {
			await expectNoSchemaIssueAsync(strictTupleAsync([optionalAsync(string()), number()]), [
				["foo", 123],
				[undefined, 123],
			]);
		});
	...
	describe("should return dataset with issues", () => {
		const schema = strictTupleAsync([optionalAsync(string()), number()], "message");
