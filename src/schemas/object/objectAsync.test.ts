...
import { type ObjectSchemaAsync, objectAsync } from "./objectAsync.ts";
...
describe("objectAsync", () => {
	...
	describe("should return dataset without issues", () => {
		...
		test("for unknown entries", async () => {
			expect(
				await objectAsync({ key1: string() })._run({ typed: false, value: { key1: "foo", key2: 123, key3: null } }, {}),
			).toStrictEqual({
				typed: true,
				value: { key1: "foo" },
			});
	...
	describe("should return dataset with nested issues", () => {
		...
		test("for missing nested entries", async () => {
			expect(await schema._run({ typed: false, value: { key: "value", nested: {} } }, {})).toStrictEqual({
				typed: false,
