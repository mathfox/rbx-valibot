...
import { type StrictObjectSchema, strictObject } from "./strictObject.ts";
...
describe("strictObject", () => {
	...
	describe("should return dataset without issues", () => {
		...
		test("for simple object", () => {
			expectNoSchemaIssue(strictObject({ key1: string(), key2: number() }), [{ key1: "foo", key2: 123 }]);
		});
	});
	...
	describe("should return dataset without nested issues", () => {
		...
		test("for nested object", () => {
			expectNoSchemaIssue(strictObject({ nested: object({ key: string() }) }), [{ nested: { key: "foo" } }]);
		});
