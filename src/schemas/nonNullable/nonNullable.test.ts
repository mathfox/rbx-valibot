...
import { type NonNullableSchema, nonNullable } from "./nonNullable.ts";
import type { NonNullableIssue } from "./types.ts";

describe("nonNullable", () => {
	describe("should return schema object", () => {
		const wrapped = nullish(string());
		const baseSchema: Omit<NonNullableSchema<NullishSchema<StringSchema<undefined>, undefined>, never>, "message"> = {
			kind: "schema",
			type: "non_nullable",
		...
		test("with undefined message", () => {
			const schema: NonNullableSchema<NullishSchema<StringSchema<undefined>, undefined>, undefined> = {
				...baseSchema,
				message: undefined,
			};
		...
		test("with string message", () => {
			expect(nonNullable(wrapped, "message")).toStrictEqual({
				...baseSchema,
				message: "message",
			} satisfies NonNullableSchema<NullishSchema<StringSchema<undefined>, undefined>, "message">);
		});
		...
		test("with function message", () => {
			...
			expect(nonNullable(wrapped, message)).toStrictEqual({
				...baseSchema,
				message,
			} satisfies NonNullableSchema<NullishSchema<StringSchema<undefined>, undefined>, typeof message>);
		});
	});
