...
import { type LooseObjectSchemaAsync, looseObjectAsync } from "./looseObjectAsync.ts";
...
describe("looseObjectAsync", () => {
	...
	describe("should return dataset without issues", () => {
		...
		test("for simple object", async () => {
			await expectNoSchemaIssueAsync(looseObjectAsync({ key1: string(), key2: number() }), [
				{ key1: "foo", key2: 123 },
			]);
		});
	...
	describe("should return dataset without nested issues", () => {
		...
		test("for nested object", async () => {
			await expectNoSchemaIssueAsync(looseObjectAsync({ nested: objectAsync({ key: string() }) }), [
				{ nested: { key: "foo" } },
			]);
		});

		test("for optional entry", async () => {
			await expectNoSchemaIssueAsync(looseObjectAsync({ key: optional(string()) }), [
				{},
				// @ts-expect-error
				{ key: undefined },
				{ key: "foo" },
		...
		test("for nullish entry", async () => {
			await expectNoSchemaIssueAsync(looseObjectAsync({ key: nullish(number()) }), [
				{},
				{ key: undefined },
				{ key: null },
				{ key: 123 },
