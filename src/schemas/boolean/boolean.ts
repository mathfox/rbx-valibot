 ...
 * Boolean schema type.
 */
export interface BooleanSchema<TMessage extends ErrorMessage<BooleanIssue> | undefined>
 ...
 * @returns A boolean schema.
 */
export function boolean<const TMessage extends ErrorMessage<BooleanIssue> | undefined>(
	message: TMessage,
): BooleanSchema<TMessage>;

export function boolean(message?: ErrorMessage<BooleanIssue>): BooleanSchema<ErrorMessage<BooleanIssue> | undefined> {
	return {
		kind: "schema",
		type: "boolean",
		reference: boolean,
		...
		message,
		_run(dataset, config) {
			if (typeIs(dataset.value, "boolean")) {
				dataset.typed = true;
			} else {
