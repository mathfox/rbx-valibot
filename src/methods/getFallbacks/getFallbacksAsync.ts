...
>(schema: TSchema): Promise<InferFallbacks<TSchema>> {
	...
	if ("entries" in schema) {
		...
		for (const [key, value] of schema.entries as unknown as Map<string, unknown>) {
			promises.push(
				(async () => {
					return [key, await getFallbacksAsync(value as BaseSchemaAsync<unknown, unknown, BaseIssue<unknown>>)];
				})(),
			);
		}
