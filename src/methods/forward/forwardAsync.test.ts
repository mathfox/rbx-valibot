...
import { forwardAsync } from "./forwardAsync";
...
describe("forwardAsync", () => {
	test("should forward issues to end of path list", async () => {
		...
		expect(
			await forwardAsync<Input, CheckIssue<Input>>(check(requirement, "message"), ["nested", 1, "key"])._run(
	...
	test("should stop forwarding if path input is undefined", async () => {
		...
		const requirement = () => false;
		expect(
			await forwardAsync<Input, CheckIssue<Input>>(check(requirement, "message"), ["nested", 6, "key"])._run(
				{ typed: true, value: input },
				{},
			),
		).toStrictEqual({
