...
describe("hash", () => {
	...
	describe("should return dataset without issues", () => {
		test("for untyped inputs", () => {
			expect(hash(["md5"])._run({ typed: false, value: null }, {})).toStrictEqual({
				typed: false,
				value: null,
			});
		});
		...
		test("for crc32 hashes", () => {
			expectNoActionIssue(hash(["crc32"]), ["3d08bb77", "d87f7e0c", "35ce1956", "fb57a171"]);
		});

		test("for crc32b hashes", () => {
			expectNoActionIssue(hash(["crc32b"]), ["7d4da703", "35ce1956", "fb57a171"]);
		});

		test("for adler32 hashes", () => {
			expectNoActionIssue(hash(["adler32"]), ["045d01c1", "0bbf02f2", "66b108b6"]);
		});
	...
	describe("should return dataset with issues", () => {
		const baseIssue: Omit<HashIssue<string>, "input" | "received" | "requirement"> = {
			kind: "validation",
			type: "hash",
			expected: null,
			message: "message",
		...
		test("for invalid md4 hashes", () => {
			expectActionIssue(hash(["md4"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{32}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid md5 hashes", () => {
			expectActionIssue(hash(["md5"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{32}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid sha1 hashes", () => {
			expectActionIssue(hash(["sha1"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{40}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid sha256 hashes", () => {
			expectActionIssue(hash(["sha256"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{64}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid sha384 hashes", () => {
			expectActionIssue(hash(["sha384"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{96}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid sha512 hashes", () => {
			expectActionIssue(hash(["sha512"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{128}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid ripemd128 hashes", () => {
			expectActionIssue(hash(["ripemd128"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{32}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid ripemd160 hashes", () => {
			expectActionIssue(hash(["ripemd160"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{40}$/iu }, [
				"",
				" ",
				"12345",
				"350de5ee5242d93e23a98966ad7ea3ce",
		...
		test("for invalid tiger128 hashes", () => {
			expectActionIssue(hash(["tiger128"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{32}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid tiger160 hashes", () => {
			expectActionIssue(hash(["tiger160"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{40}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid tiger192 hashes", () => {
			expectActionIssue(hash(["tiger192"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{48}$/iu }, [
				"",
				" ",
				"12345",
		...
		test("for invalid crc32 hashes", () => {
			expectActionIssue(hash(["crc32"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{8}$/iu }, [
				"",
				" ",
				"12345",
				"3d08bq77",
		...
		test("for invalid crc32b hashes", () => {
			expectActionIssue(hash(["crc32b"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{8}$/iu }, [
				"",
				" ",
				"12345",
				"35cer956",
		...
		test("for invalid adler32 hashes", () => {
			expectActionIssue(hash(["adler32"], "message"), { ...baseIssue, requirement: /^[a-f0-9]{8}$/iu }, [
				"",
				" ",
				"12345",
				"045dx1c1",
